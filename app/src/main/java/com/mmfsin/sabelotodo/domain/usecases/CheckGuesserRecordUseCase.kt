...
class CheckGuesserRecordUseCase @Inject constructor(private val repository: IDashboardRepository) :
    BaseUseCase<CheckGuesserRecordUseCase.Params, Record?>() {
    ...
    override suspend fun execute(params: Params): Record? {
        return try {
            ...
            val isRecord = if (points > 0 && points > record) {
                repository.updateGuesserRecord(params.categoryId, points)
                true
            } else false
